...
impl WebsiteRegenerator {
    ...
    pub fn regenerate(&self, original_code: &str) -> Result<String> {
        log::debug!("Regenerating website code (length: {})", original_code.len());
        ...
        for _ in 0..self.max_len {
            ...
            // 创建ONNX输入
            let src_array = ndarray::Array2::from_shape_vec(
                (1, src_len),
                src_tokens.clone(),
            )?;
            let tgt_array = ndarray::Array2::from_shape_vec(
                (1, self.max_len),
                tgt_padded,
            )?;
        ...
        log::debug!("Regeneration complete (output length: {})", regenerated.len());
