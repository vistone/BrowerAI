...
use browerai::learning::{
    ...
    DeobfuscationStrategy, GenerationRequest, JsDeobfuscator, ObfuscationAnalysis,
};
...
fn main() -> anyhow::Result<()> {
    ...
    println!("╔══════════════════════════════════════════════════════════════╗");
    println!("║    BrowerAI Comprehensive Enhancement Demo                 ║");
    println!("╚══════════════════════════════════════════════════════════════╝\n");

    // ========== Part 1: Code Generation ==========
    println!("\n═══ Part 1: AI-Powered Code Generation ═══\n");
    demo_code_generation()?;

    // ========== Part 2: JS Deobfuscation ==========
    println!("\n═══ Part 2: Advanced JS Deobfuscation ═══\n");
    demo_js_deobfuscation()?;

    // ========== Part 3: Continuous Learning ==========
    println!("\n═══ Part 3: Continuous Learning Loop ═══\n");
...
fn demo_code_generation() -> anyhow::Result<()> {
    ...
    html_constraints.insert(
        "content".to_string(),
        "This page was generated by AI!".to_string(),
    );
    ...
    println!(
        "✅ Generated HTML ({} tokens, confidence: {:.2}):",
        html_result.metadata.token_count, html_result.confidence
    );
    ...
    println!(
        "✅ Generated CSS ({} tokens, confidence: {:.2}):",
        css_result.metadata.token_count, css_result.confidence
    );
    println!("{}\n", truncate_code(&css_result.code, 200));

    // Generate JavaScript
    println!("⚙️  Generating JavaScript function...");
    ...
    js_constraints.insert(
        "description".to_string(),
        "Process and validate data".to_string(),
    );
    js_constraints.insert(
        "body".to_string(),
        "const result = data.map(x => x * 2);".to_string(),
    );
    ...
    println!(
        "✅ Generated JavaScript ({} tokens, confidence: {:.2}):",
        js_result.metadata.token_count, js_result.confidence
    );
...
fn demo_js_deobfuscation() -> anyhow::Result<()> {
    ...
    let test_cases = vec![
        ("Minified Code", "var a=1;var b=2;var c=a+b;console.log(c);"),
        (
            "Hex Encoding",
    ...
    for (name, obfuscated_code) in test_cases {
        ...
        println!("✅ Deobfuscation completed!");
        println!("   Steps applied: {}", result.steps.len());
        println!(
            "   Readability improvement: {:.2} -> {:.2}",
            result.improvement.readability_before, result.improvement.readability_after
        );
...
fn demo_continuous_learning() -> anyhow::Result<()> {
    println!("🔄 Starting continuous learning loop (3 iterations)...\n");
        ...
        for event in events {
            match event {
                ...
                browerai::learning::continuous_loop::LearningEvent::ModelUpdated(count) => {
                    println!("\n   ✅ Model updated (update #{})", count);
                }
                browerai::learning::continuous_loop::LearningEvent::CodeGenerated(code_type) => {
                    println!("   🔨 Generated {:?} code", code_type);
                }
                browerai::learning::continuous_loop::LearningEvent::IterationCompleted(
                    iter,
                    duration,
                ) => {
                    println!(
                        "\n   ⏱️  Iteration {} completed in {:.2}ms",
                        iter,
...
fn print_analysis(analysis: &ObfuscationAnalysis) {
    ...
    println!(
        "      Max Nesting: {}",
        analysis.complexity.max_nesting_depth
    );
    println!(
        "      Avg Var Name Length: {:.1}",
        analysis.complexity.avg_var_name_length
    );
