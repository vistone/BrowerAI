...
impl JsParser {
    ...
    fn extract_event_handlers(&self, code: &str) -> Vec<EventHandler> {
        ...
        let listener_re = regex::Regex::new(
            r"addEventListener\s*\(\s*['\x22](\w+)['\x22]\s*,\s*(\w+)"
        ).unwrap();
        ...
        // onclick="..."
        let onclick_re = regex::Regex::new(r"onclick\s*=\s*[\x27\x22]([^\x27\x22]+)[\x27\x22]").unwrap();
    ...
    fn extract_api_calls(&self, code: &str) -> Vec<ApiCall> {
        ...
        let fetch_re = regex::Regex::new(
            r"fetch\s*\(\s*[\x27\x22]([^\x27\x22]+)[\x27\x22]"
        ).unwrap();
        ...
        // XMLHttpRequest
        let xhr_re = regex::Regex::new(r"open\s*\(\s*[\x27\x22](\w+)[\x27\x22]\s*,\s*[\x27\x22]([^\x27\x22]+)[\x27\x22]").unwrap();
