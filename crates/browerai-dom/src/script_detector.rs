...
impl DomScriptCollector {
    ...
    pub fn process_script(&self, id: String, source: ScriptSource) -> Result<ScriptElement> {
        let content = match &source {
            ...
            ScriptSource::External { content, .. } => content.clone().unwrap_or_else(|| {
                debug!("⚠️  外部脚本无可用内容，跳过框架检测: {}", id);
                String::new()
            }),
            ScriptSource::Module { content, .. } => content.clone().unwrap_or_else(|| {
    ...
    pub fn process_scripts(
        &self,
        scripts: Vec<(String, ScriptSource)>,
        ...
        let scripts = vec![
            ...
            collector
                .process_script(
                    "v1".to_string(),
                    ScriptSource::Inline {
                        content: "import { createApp } from 'vue';".to_string(),
                    },
                )
                .unwrap(),
        ];
