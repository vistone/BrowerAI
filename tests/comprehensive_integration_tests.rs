...
use browerai::learning::{
    CodeGenerator, GenerationRequest, CodeType, GeneratedCode,
...
fn test_html_generation_and_validation() {
    ...
    let dom = parser.parse(&result.code).expect("Generated HTML is invalid");
    let text = parser.extract_text(&dom);
    
    // Verify content is present
    assert!(text.contains("Test Page") || text.contains("Hello World"), 
        "Generated HTML should contain expected content");
    assert!(result.code.contains("<!DOCTYPE html>"), "Should have DOCTYPE");
    assert!(result.code.contains("<html>"), "Should have html tag");
    assert!(result.code.contains("</html>"), "Should have closing html tag");
}
...
fn test_css_generation_and_validation() {
    ...
    let rules = parser.parse(&result.code).expect("Generated CSS is invalid");
    
    // Verify CSS structure
    assert!(!rules.is_empty(), "CSS should have rules");
    assert!(result.code.contains("Arial") || result.code.contains("sans-serif"), 
...
fn test_js_generation_and_validation() {
    ...
    assert!(result.code.contains("function") || result.code.contains("calculateSum"),
...
fn test_deobfuscation_actually_improves_code() {
    ...
    let parser = JsParser::new();
    let ast = parser.parse(&result.code).expect("Deobfuscated code should be valid");
...
fn test_learn_infer_generate_cycle_with_html() {
    ...
    let regenerated = generator.generate(&new_request).expect("Regeneration failed");
    
    // Validate the regenerated code
    let new_dom = parser.parse(&regenerated.code).expect("Regenerated parse failed");
    let new_text = parser.extract_text(&new_dom);
    assert!(!new_text.is_empty(), "Should extract text from regenerated HTML");
}
...
fn test_deobfuscation_preserves_functionality() {
    ...
    // Deobfuscate
    let deobfuscated_result = deobfuscator.deobfuscate(
        original,
        DeobfuscationStrategy::Comprehensive
    ).expect("Deobfuscation failed");
    
    // Parse deobfuscated
    let deobf_ast = parser.parse(&deobfuscated_result.code)
...
fn test_generated_code_standards_compliance() {
    ...
    let html = generator.generate(&html_request).expect("HTML generation failed");
    assert!(html.code.starts_with("<!DOCTYPE html>"), "HTML5 DOCTYPE required");
    ...
    let css = generator.generate(&css_request).expect("CSS generation failed");
    // CSS should have proper selector and property format
    assert!(css.code.contains("{") && css.code.contains("}"), "CSS should have proper blocks");
    assert!(css.code.contains(":") || css.code.contains(";"), "CSS should have properties");
    ...
    let js = generator.generate(&js_request).expect("JS generation failed");
    // Should be valid JavaScript
    let parser = JsParser::new();
    assert!(parser.validate(&js.code).expect("Validation check failed"),
...
fn test_multiple_obfuscation_techniques_detection() {
    ...
    assert!(analysis.obfuscation_score > 0.0, "Should detect obfuscation");
    assert!(analysis.techniques.len() >= 2, "Should detect multiple techniques");
    
    // Should provide suggestions
    assert!(!analysis.suggestions.is_empty(), "Should provide suggestions");
}
...
fn test_end_to_end_website_simulation() {
    ...
    let html = generator.generate(&GenerationRequest {
        code_type: CodeType::Html,
        description: "basic page".to_string(),
        constraints: {
    ...
    let css = generator.generate(&GenerationRequest {
        code_type: CodeType::Css,
        description: "basic styling".to_string(),
        constraints: HashMap::new(),
    }).expect("CSS generation failed");
    
    let js = generator.generate(&GenerationRequest {
        code_type: CodeType::JavaScript,
        description: "function".to_string(),
        constraints: HashMap::new(),
    ...
    assert!(!html_parser.extract_text(&html_dom).is_empty(), "HTML should have content");
    assert!(!css_rules.is_empty() || css.code.contains("body"), "CSS should have content");
    assert!(js_ast.is_valid, "JS should be valid");
    
    // 4. Infer patterns and regenerate
    let regenerated_html = generator.generate(&GenerationRequest {
        code_type: CodeType::Html,
        description: "basic page".to_string(),
        constraints: {
            let mut m = HashMap::new();
    ...
    let regenerated_dom = html_parser.parse(&regenerated_html.code)
        .expect("Regenerated HTML parse failed");
    assert!(!html_parser.extract_text(&regenerated_dom).is_empty(),
