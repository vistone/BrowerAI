...
fn test_hex_string_decoding() {
    ...
    assert!(
        result.code.contains("Hello") || !result.code.contains("\\x"),
        "Hex strings should be decoded. Got: {}",
        result.code
    );
}
...
fn test_variable_renaming_transformation() {
    ...
    assert!(
        result.code.contains("var") && result.code.len() > code.len() - 10,
        "Variables should be renamed. Got: {}",
        result.code
    );
}
...
fn test_dead_code_removal() {
    ...
    assert!(
        !result.code.contains("if(false)") || result.code.len() < code.len(),
        "Dead code should be removed. Got: {}",
        result.code
    );
}
...
fn test_comprehensive_deobfuscation() {
    ...
    assert!(
        parser.validate(&result.code).unwrap(),
