...
//!
//! 从真实网站获取混淆的 JavaScript 代码，测试反混淀功能
//! 验证反混淀后的代码是否可以执行

use browerai::learning::{DeobfuscationStrategy, JsDeobfuscator, WebsiteDeobfuscationVerifier, ExecutionValidator};
...
"#;
    ...
    // 代码应该更可读
    assert!(
        result.improvement.readability_after > result.improvement.readability_before,
        "Deobfuscated code should be more readable"
    );
}
...
fn test_real_world_minified_libraries_execution_validation() {
    ...
    for url in urls {
        let result = verifier
            ...
            .expect("Failed to fetch and deobfuscate real-world JS");

        assert!(result.original_size > 1024, "Should fetch non-trivial JS from {}", url);
        assert!(result.success, "Deobfuscation should succeed for {}", url);
        assert!(result.is_valid, "Deobfuscated code should be valid for {}", url);
        ...
        assert!(
            ...
            "Execution validator should mark syntax valid for {}", url
        );
        assert!(exec_result.deobfuscation_success, "Execution validator should see deobfuscation success for {}", url);
    }
}
...
"#;
    ...
    assert!(
        result.improvement.readability_after >= result.improvement.readability_before,
        "Deobfuscation should maintain or improve readability"
    );
}
...
"#;
    ...
    // 原始代码有效
    assert!(
        parser.validate(react_code).unwrap_or(false),
        "React code should be valid"
    );

    // 反混淀（虽然 React 代码不算混淀，但测试兼容性）
    let result = deobf
    ...
    assert!(
        parser.validate(&result.code).unwrap_or(false),
        "Deobfuscated React code should be valid"
    );
}
...
"#;
    ...
    // 反混淀
    let result = deobf
        .deobfuscate(flattened_code, DeobfuscationStrategy::ControlFlowSimplification)
        .unwrap();
...
"#;
    ...
    assert!(is_valid, "Deobfuscated code must be valid JavaScript: {}", result_deobf.code);
...
"#;
    ...
    assert!(
        result.improvement.readability_after >= 0.0 && 
        result.improvement.readability_after <= 1.0,
        "Readability scores should be between 0 and 1"
    );
